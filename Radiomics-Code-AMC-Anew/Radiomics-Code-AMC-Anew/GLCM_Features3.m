...
% Entropy: [2]                              *(out.entro)*
% Homogeneity: matlab (=IDM)                *(out.homom)*
% Homogeneity: [2]                          *(out.homop)* = idmmen
...
% out.idmmen = zeros(1,size_glcm_3); % Inverse Difference Moment (IDM)
    ...
    for i = 1:size_glcm_1
        for j = 1:size_glcm_2
            ...
            out.cshad(k) = out.cshad(k) + (((i + j - u_x(k) - u_y(k))^3)*...
                glcm(i,j,k));
            out.cstend(k) = out.cstend(k) + (((i + j - u_x(k) - u_y(k))^2)*...
                glcm(i,j,k));
        end
    end
